...
public class WVCGuiSettings extends GuiScreen {
    ...
    private void drawColorPicker(ColorSetting s, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < sz; i++) for (int j = 0; j < sz; j++) drawRect(px + i, py + j, px + i + 1, py + j + 1, Color.HSBtoRGB(h[0], (float)i/sz, 1.0f - (float)j/sz));
        int hx = px + sz + 15;
        for (int i = 0; i < sz; i++) drawRect(hx, py + i, hx + 15, py + i + 1, Color.HSBtoRGB((float)i/sz, 1.0f, 1.0f));
    ...
    protected void mouseClicked(int mouseX, int mouseY, int button) throws IOException {
        ...
        for (int i = 0; i < tabs.length; i++) {
            ...
            if (mouseX >= tx && mouseX <= tx + 100 && mouseY >= py + 10 && mouseY <= py + 35) {
                ...
                mc.thePlayer.playSound("random.click", 0.5F, 1.0F); return;
            }
        }

        // Edit HUD button
        if (mouseX >= px + PANEL_W - 95 && mouseY >= py + 12 && mouseY <= py + 34) {
            mc.displayGuiScreen(new GuiEditHUD()); return;
        }
        ...
        if (currentTab.equals("MODS")) {
            ...
            for (int i = 0; i < themes.length; i++) {
                ...
                if (mouseX >= tx && mouseX <= tx + 120 && mouseY >= ty && mouseY <= ty + 30) {
                    ...
                    mc.thePlayer.playSound("random.click", 0.5F, 1.2F); return;
                }
            }
        } else if (currentTab.equals("PROFILES")) {
            ...
            for (String p : profiles) {
                if (mouseX >= sx && mouseX <= sx + 200 && mouseY >= cy && mouseY <= cy + 35) {
                    pm.switchProfile(p); mc.thePlayer.playSound("random.click", 0.5F, 1.0F); return;
                }
                cy += 42;
            }
            if (mouseX >= sx && mouseX <= sx + 130 && mouseY >= cy + 10 && mouseY <= cy + 38) {
                pm.createProfile("Profile " + (profiles.size() + 1));
                mc.thePlayer.playSound("random.click", 0.5F, 1.5F); return;
            }
        }
        super.mouseClicked(mouseX, mouseY, button);
    }

    @Override protected void mouseClickMove(int x, int y, int b, long t) { if (selectedModule != null) handleInt(selectedModule, x, y, false); }

    private void handleInt(WVCModule m, int mx, int my, boolean f) {
        int sx = width/2 - PANEL_W/2 + SIDEBAR_W + 30;
        int sy = height/2 - PANEL_H/2 + TOP_BAR_H + 40 - detailScrollOffset;
        sy += 35;
        if (m instanceof CrosshairModule) {
            if (f && mx >= sx && mx <= sx + 110 && my >= sy && my <= sy + 22) {
                mc.displayGuiScreen(new GuiCrosshairEditor(this, (CrosshairModule) m)); return;
            }
            sy += 40;
        }
        for (Setting s : m.getSettings()) {
            if (s instanceof BooleanSetting) {
                if (f && mx >= width/2 + PANEL_W/2 - 60 && my >= sy && my <= sy + 20) ((BooleanSetting) s).toggle();
                sy += 32;
            } else if (s instanceof SliderSetting) {
    ...
    private void drawRoundedRect(int x1, int y1, int x2, int y2, int r, int col) {
        Gui.drawRect(x1 + r, y1, x2 - r, y2, col);
        Gui.drawRect(x1, y1 + r, x2, y2 - r, col);
    ...
    private void drawRoundedRectTop(int x1, int y1, int x2, int y2, int r, int col) {
        Gui.drawRect(x1, y1 + r, x2, y2, col);
        Gui.drawRect(x1 + r, y1, x2 - r, y1 + r, col);
        fillCircle(x1 + r, y1 + r, r, col); fillCircle(x2 - r, y1 + r, r, col);
    }

    private void drawRoundedRectLeft(int x1, int y1, int x2, int y2, int r, int col) {
        Gui.drawRect(x1 + r, y1, x2, y2, col);
        Gui.drawRect(x1, y1 + r, x1 + r, y2 - r, col);
    ...
    private void enableScissor(int x, int y, int width, int height) {
        ...
        GL11.glScissor(x * s, mc.displayHeight - (y + height) * s, width * s, height * s);
    }
    ...
    @Override public void onGuiClosed() { WVCMod.INSTANCE.getProfileManager().saveCurrentProfile(); super.onGuiClosed(); }
