...
public class WVCGuiSettings extends GuiScreen {
    ...
    public void func_73863_a(int mouseX, int mouseY, float partialTicks) {
        ...
        func_146276_q_();
        func_73734_a(px, py, px + PANEL_W, py + PANEL_H, COL_BG);

        // --- トップバー ---
        func_73734_a(px, py, px + PANEL_W, py + TOP_BAR_H, COL_SIDEBAR);
        String[] tabs = {
        ...
        func_73734_a(sx, py + 13, sx + 140, py + 37, 0x22FFFFFF);
        ...
        if (selectedModule != null) {
            ...
            if (currentTab.equals(dev.windv.wvc.util.LanguageManager.get("gui.mods"))) {
                func_73734_a(px, contentY, px + SIDEBAR_W, py + PANEL_H, COL_SIDEBAR);
    ...
    private void drawModuleList(int x, int y, int w, int h, int mx, int my) {
        ...
        for (int i = 0; i < mods.size(); i++) {
            ...
            func_73734_a(cx, cy, cx + CARD_W, cy + CARD_H, hov ? COL_CARD_HOVER : COL_CARD);
            if (m.isEnabled()) func_73734_a(cx, cy, cx + 4, cy + CARD_H, getAccentColor());
    ...
    private void drawModuleSettings(WVCModule m, int x, int y, int w, int h, int mx, int my) {
        ...
        if (m instanceof CrosshairModule) {
            func_73734_a(sx, sy, sx + 120, sy + 24, getAccentColor());
    ...
    private void drawColorPicker(ColorSetting s, int x, int y, int w, int mx, int my) {
        ...
        // プレビュー
        func_73734_a(px, py, px + 25, py + boxSize, s.getColor() | 0xFF000000);
        ...
        for (int col = 0; col < boxSize; col++) {
            ...
            for (int row = 0; row < boxSize; row++) {
                ...
                func_73734_a(bx + col, py + row, bx + col + 1, py + row + 1, c);
            }
        }
        ...
        func_73734_a(indX - 3, indY - 1, indX + 3, indY + 1, 0xFFFFFFFF);
        func_73734_a(indX - 1, indY - 3, indX + 1, indY + 3, 0xFFFFFFFF);

        // --- 縦型 Hue バー ---
        int hx = bx + boxSize + 10;
        for (int i = 0; i < boxSize; i++) {
            int hCol = Color.HSBtoRGB((float) i / (boxSize - 1), 1.0f, 1.0f) | 0xFF000000;
            func_73734_a(hx, py + i, hx + 15, py + i + 1, hCol);
        }
    ...
    private void handleColorClick(ColorSetting s, int x, int y, int w, int mx, int my) {
        ...
        if (mx >= hx && mx <= hx + 15 && my >= py && my <= py + boxSize) {
            hsb[0] = MathHelper.func_76131_a((float)(my - py) / boxSize, 0, 0.999f);
        } else if (mx >= bx && mx <= bx + boxSize && my >= py && my <= py + boxSize) {
            hsb[1] = MathHelper.func_76131_a((float)(mx - bx) / boxSize, 0, 1);
            hsb[2] = MathHelper.func_76131_a(1.0f - (float)(my - py) / boxSize, 0, 1);
        } else { return; }
    ...
    protected void func_73869_a(char typedChar, int keyCode) throws IOException {
        if (listeningSearch) {
            ...
            else if (ChatAllowedCharacters.func_71566_a(typedChar)) { searchQuery += typedChar; }
    ...
    protected void func_73864_a(int mx, int my, int btn) throws IOException {
        int px = field_146294_l / 2 - PANEL_W / 2; int py = field_146295_m / 2 - PANEL_H / 2;
        int ehX = px + PANEL_W - 110;
        if (mx >= ehX && mx <= ehX + 90 && my >= py + 13 && my <= py + 37) { field_146297_k.func_147108_a(new GuiEditHUD()); return; }
        int sx = px + PANEL_W - 265;
    ...
    @Override
    protected void func_146273_a(int mx, int my, int btn, long time) {
        if (selectedModule != null) {
            int px = field_146294_l / 2 - PANEL_W / 2; int py = field_146295_m / 2 - PANEL_H / 2;
    ...
    private void handleScrolling(int mx, int my) {
        ...
        if (wheel != 0) {
            if (selectedModule != null) detailScrollOffset = MathHelper.func_76125_a(detailScrollOffset - (wheel / 2), 0, detailMaxScroll);
            else scrollOffset = MathHelper.func_76125_a(scrollOffset - (wheel / 2), 0, maxScrollOffset);
        }
    }
    ...
    private void drawGlobalSettings(int x, int y, int w, int h, int mx, int my) {
        ...
        for (int i = 0; i < 6; i++) {
            func_73734_a(sx + i*45, sy, sx + i*45 + 35, sy + 35, colors[i]);
            if (getAccentColor() == colors[i]) func_73734_a(sx + i*45 + 4, sy + 4, sx + i*45 + 31, sy + 31, 0x66FFFFFF);
        }
        ...
        func_73734_a(sx, sy, sx + 90, sy + 24, en ? getAccentColor() : 0x22FFFFFF);
        ...
        func_73734_a(sx + 105, sy, sx + 195, sy + 24, jp ? getAccentColor() : 0x22FFFFFF);
    ...
    private void drawProfiles(int x, int y, int w, int h, int mx, int my) {
        ...
        for (String p : pm.getProfileList()) {
            ...
            func_73734_a(sx, sy, sx + 220, sy + 35, sel ? getAccentColor() : 0x22FFFFFF);
    ...
    private void drawCategorySidebar(int x, int y, int w, int h, int mx, int my) {
        ...
        for (String c : cats) {
            ...
            if (sel) { func_73734_a(x, cy, x + SIDEBAR_W, cy + 30, 0x15FFFFFF); func_73734_a(x, cy, x + 3, cy + 30, getAccentColor()); }
    ...
    private void drawToggleSwitch(int x, int y, boolean on) {
        func_73734_a(x, y, x + 30, y + 15, on ? getAccentColor() : 0xFF333333);
        func_73734_a(on ? x + 16 : x + 2, y + 2, on ? x + 28 : x + 14, y + 13, 0xFFFFFFFF);
    }
    ...
    private void drawSlider(SliderSetting s, int x, int y, int w, int mx, int my) {
        ...
        func_73734_a(x, y + 22, x + w, y + 25, 0x44FFFFFF);
        float p = (float) ((s.getValue() - s.getMin()) / (s.getMax() - s.getMin()));
        func_73734_a(x, y + 22, x + (int)(w * p), y + 25, getAccentColor());
        func_73734_a(x + (int)(w * p) - 4, y + 18, x + (int)(w * p) + 4, y + 29, 0xFFFFFFFF);
    }

    private int getAccentColor() { return WVCMod.INSTANCE.getConfig().getThemeColor(); }
    private void enableScissor(int x, int y, int w, int h) {
        int f = new ScaledResolution(field_146297_k).func_78325_e();
        GL11.glEnable(GL11.GL_SCISSOR_TEST);
        GL11.glScissor(x * f, (field_146297_k.field_71440_d - (y + h) * f), w * f, h * f);
    }
